...
def create_folium_map(stations_close_municipality, center, comune):
    ...
    for _, row in stations_close_municipality.iterrows():
        popup = f'Price: {row["prezzo"]}\nAddress: {row["Indirizzo"].lower().capitalize()}\nComune: {row["Comune"].lower().capitalize()}'
        fg.add_child(
...
if __name__ == '__main__':
    ...
    st.divider()
    st.subheader("This project aims to find the best oil station where to get petrol in a location of your choice:fuelpump:",
    ...
    st_data = st_folium(map_price, feature_group_to_add=fg, center=location, width = 725, zoom=12)
    
    st.caption('Racoons are too cute :raccoon::raccoon:')